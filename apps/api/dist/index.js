...
import { AccessLevel, ActivityType, ChildGender, ChildCaregiverRole, PrismaClient, RecommendationStatus, SessionStatus, } from '@prisma/client';
import { z } from 'zod';
...
function setAuthSessionCookie(res, payload, maxAgeSeconds = SESSION_DURATION_SECONDS) {
...
async function requireChildSession(req, res) {
...
const prodAllowlist = ['https://aprende-con-coco.vercel.app', process.env.CORS_ORIGIN].filter(Boolean);
...
async function refreshSessionStats(sessionId) {
    ...
    const responseHelpCount = responses.reduce((total, response) => total + (response.helpUsed ? 1 : 0) + response.hintsUsed, 0);
    const responseAudioCount = responses.reduce((total, response) => total + response.audioReplayCount, 0);
    const eventHelpCount = events.filter((event) => event.type === 'hint_used').length;
    const eventAudioCount = events.filter((event) => ['audio_replayed', 'narration_replayed'].includes(event.type)).length;
...
async function syncChildActivityDailyStat(childId, activityId, statDate) {
    ...
    const reactionValues = sessions.flatMap((session) => session.responses
        .map((response) => response.reactionMs)
        .filter((reactionMs) => typeof reactionMs === 'number' && reactionMs > 0));
    ...
    const avgReactionMs = reactionValues.length > 0
        ? Math.round(reactionValues.reduce((sum, reactionMs) => sum + reactionMs, 0) / reactionValues.length)
        : null;
    const engagementScore = roundMetric(clamp01(completedRatio * 0.45 +
        (accuracyRate ?? 0) * 0.35 +
        Math.max(0, 1 - totalHelpUsed / Math.max(totalResponses, 1)) * 0.2));
    const preferenceScore = roundMetric(clamp01(completedRatio * 0.4 +
...
async function syncChildSkillSnapshot(childId, skillId, snapshotDate) {
    ...
    const confidenceScore = roundMetric(clamp01(accuracyRate * 0.6 + Math.min(1, attemptsCount / 12) * 0.4));
    const masteryScore = roundMetric(clamp01(accuracyRate * 0.7 +
...
async function syncSessionProgressAggregates(sessionId) {
...
async function buildChildDashboard(childId) {
    const child = await prisma.child.findUnique({
        where: { id: childId },
        include: {
            caregivers: {
    ...
    const overview = dailyStats.reduce((accumulator, stat) => {
        ...
        accumulator.activities[stat.activity.code] = {
            ...
            preferenceScore: (accumulator.activities[stat.activity.code]?.preferenceScore ?? 0) +
    ...
    const favoriteActivity = Object.values(overview.activities).sort((left, right) => {
    ...
    return {
        ...
        overview: {
            ...
            accuracyRate: overview.totalResponses > 0
...
app.post('/auth/login-child', async (req, res) => {
...
app.post('/auth/unlock-admin', async (req, res) => {
    const auth = await requireChildSession(req, res);
    if (!auth) {
        return;
    }
...
app.post('/auth/lock-admin', async (req, res) => {
    const auth = await requireChildSession(req, res);
    if (!auth) {
        return;
    }
...
app.get('/children', async (req, res) => {
    ...
    res.json(list.map(({ sessions, recommendations, ...child }) => ({
        ...child,
        _count: {
...
app.post('/sessions/:sessionId/responses', async (req, res) => {
    const auth = await requireChildSession(req, res);
    if (!auth) {
        return;
    }
    ...
    const responses = await prisma.$transaction(parsed.data.responses.map((response) => prisma.activityResponse.upsert({
        where: {
            sessionId_sequence: {
                sessionId,
...
app.post('/sessions/:sessionId/events/batch', async (req, res) => {
    const auth = await requireChildSession(req, res);
    if (!auth) {
        return;
    }
...
app.post('/sessions/:sessionId/finish', async (req, res) => {
    const auth = await requireChildSession(req, res);
    if (!auth) {
        return;
    }
    ...
    const computedScore = refreshedSession.totalResponses > 0
        ? Number(((refreshedSession.correctResponses / refreshedSession.totalResponses) * 100).toFixed(1))
        : null;
...
app.get('/children/:childId/dashboard', async (req, res) => {
    const auth = await requireAdminSession(req, res);
    if (!auth) {
        return;
    }
...
app.get('/progress/:childId', async (req, res) => {
    ...
    const byActivity = stats.reduce((accumulator, stat) => {
    ...
    const rows = Object.values(byActivity).map((row) => ({
        ...
        accuracyRate: row.totalResponses > 0 ? Number((row.correctResponses / row.totalResponses).toFixed(2)) : null,
        avgMs: row.avgReactionSamples.length > 0
            ? Math.round(row.avgReactionSamples.reduce((total, value) => total + value, 0) /
